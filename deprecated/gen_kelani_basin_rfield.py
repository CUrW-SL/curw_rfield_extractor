...
def create_rfield(connection, wrf_model, version, sim_tag, timestamp, rfield_home, fgt):
...
#############################
def gen_rfield_d03_kelani_basin(wrf_model, version, sim_tag, rfield_home, fgt):
        ...
        if end_time > now:
            ...
            while timestamp <= end_time:
                create_rfield(connection=connection, wrf_model=wrf_model, version=version, sim_tag=sim_tag,
                              timestamp=timestamp, rfield_home=rfield_home, fgt=fgt)
...
def usage():
    usageText = """
    Usage: ./gen_kelani_basin_rfield.py -m WRF_X1,WRF_X2,WRF_X3 -v vX -s "evening_18hrs"
...
if __name__=="__main__":
    ...
    try:
        ...
        if sim_tag.split("_")[0] == 'dwrf':
            rfield_home = "{}/dwrf/{}/{}/{}/kelani_basin/rfield".format(root_directory, version, gfs_run,
                                                                          gfs_data_hour)
            bucket_rfield_home = "{}/dwrf/{}/{}/{}/{}/rfield/kelani_basin".format(bucket_root, version, gfs_run,
                                                                                 gfs_data_hour,
                                                                                 day)
        else:
            rfield_home = "{}/wrf/{}/{}/{}/kelani_basin/rfield".format(root_directory, version, gfs_run,
                                                                          gfs_data_hour)
        ...
        try:
            os.system("cp /home/uwcc-admin/curw_rfield_extractor/kelani_basin_xy.csv {}/xy.csv".format(rfield_home))
        except Exception:
            pass
        ...
        results = mp_pool.starmap(gen_rfield_d03_kelani_basin,
                                        [(wrf_model, version, sim_tag, rfield_home, fgt) for wrf_model in wrf_model_list])
